...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[calc(100dvh-64px)] -mt-8 -mx-4 sm:-mx-6 lg:-mx-8 bg-gradient-to-br from-teal-700 via-teal-600 to-cyan-500 flex items-center justify-center px-4 py-12">
      {/* Decorative blobs */}
      <div className="absolute -top-20 -left-20 w-80 h-80 bg-teal-400/30 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white/95 dark:bg-teal-950/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl shadow-teal-900/30 border border-white/30 dark:border-teal-700/30">
          <div className="mb-8">
            <h1 className="text-2xl font-extrabold text-teal-900 dark:text-white tracking-tight mb-1">
              Рады видеть вас снова
            </h1>
            <p className="text-teal-400 dark:text-teal-400 text-sm">Войдите, чтобы найти нового пушистого друга</p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="email" className="text-teal-700 dark:text-teal-300 text-sm font-medium">Email</Label>
              <Input
                id="email" type="email" value={email}
                onChange={e => setEmail(e.target.value)}
                placeholder="you@example.com" required
                className="h-11 rounded-xl border-teal-200 dark:border-teal-700 dark:bg-teal-900/40 focus:border-teal-400 focus:ring-teal-400"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="password" className="text-teal-700 dark:text-teal-300 text-sm font-medium">Пароль</Label>
              <Input
            ...
            <Button
              ...
              className="w-full h-11 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 active:scale-[0.98] transition-all gap-2 rounded-xl text-base font-semibold mt-1 border-0 shadow-sm shadow-teal-300 dark:shadow-teal-900/50"
            >
        ...
        <p className="text-center text-teal-200/80 text-sm mt-6">
          Нет аккаунта?{' '}
          <Link href="/signup" className="text-white font-semibold hover:text-teal-100 transition-colors">
            Зарегистрироваться
          </Link>
        </p>
      </div>
